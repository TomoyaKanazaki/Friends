	...
	if (m_pTargetP != NULL)
	{
		...
		{// ←キーが押された,左移動
			...
			{// A+W,左上移動
				...
				pos.x += sinf(-D3DX_PI * MOVE_LRDW + Camerarot.y) * fMove;
				pos.z += cosf(-D3DX_PI * MOVE_LRDW + Camerarot.y) * fMove;
			}
			else if (pInputKeyboard->GetPress(DIK_DOWN) == true)
			{// A+S,左下移動

				pos.x += sinf(-D3DX_PI * MOVE_LRUP + Camerarot.y) * fMove;
				pos.z += cosf(-D3DX_PI * MOVE_LRUP + Camerarot.y) * fMove;
			}
			else
			{// A,左移動

				pos.x += sinf(-D3DX_PI * MOVE_LR + Camerarot.y) * fMove;
				pos.z += cosf(-D3DX_PI * MOVE_LR + Camerarot.y) * fMove;
			}
		}
		...
		{// Wが押された、奥移動
			...
			pos.x += sinf(Camerarot.y) * fMove;
			pos.z += cosf(Camerarot.y) * fMove;
		}
		else if (pInputKeyboard->GetPress(DIK_DOWN) == true)
		{// Sが押された、手前移動

			pos.x += sinf(D3DX_PI + Camerarot.y) * fMove;
			pos.z += cosf(D3DX_PI + Camerarot.y) * fMove;
		}
