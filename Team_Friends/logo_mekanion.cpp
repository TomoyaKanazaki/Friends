...
void CLogo_Meka::Update(void)
{
	...
	CManager::GetInstance()->GetDebugProc()->Print("ロゴの角度 : %f\n", rot.x);
	CManager::GetInstance()->GetDebugProc()->Print("ロゴの位置 : %f, %f, %f\n", pos.x, pos.y, pos.z);
	CManager::GetInstance()->GetDebugProc()->Print("ロゴの仮想位置 : %f, %f, %f\n", m_posVirtual.x, m_posVirtual.y, m_posVirtual.z);
